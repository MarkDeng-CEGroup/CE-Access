...
void Google()
{
			...
			if (GetAsyncKeyState(VK_F10))
			{
				...
				while (true)
				{
					...
					if (GetAsyncKeyState(VK_F2))
					{
						...
						system("cls");
						if (l == 'C')
							cout << "如果您输入了网址，我们将执行以下哪个操作？\n\nF7:访问网址并记住该选择\nF8:不访问网址并记住该选择\nF9:每次询问";
						if (l == 'S')
							cout << "If you enter a URL, which of the following actions will we perform? \n\nF7:Visit the URL and remember the selection \nF8:Do not visit the URL and remember the selection \nF9:Ask each time";
						if (l == 'K')
							cout << "If you enter a web address, which of the following actions will we perform? \n\nF7:Visit the URL and remember the selection \nF8:Do not visit the URL and remember the selection \nF9:Ask each time";
						if (l == 'J')
							cout << "URLを入力した場合、次のどのアクションを実行しますか？\n\nF7：URLにアクセスして選択を覚えてください\nF8：URLにアクセスせずに選択を覚えてください\nF9：毎回質問してください";
						while (true)
						{
							if (GetAsyncKeyState(VK_F7))
							{
    ...
    if (in[0]=='h'&&in[1]=='t'&&in[2]=='t'&&in[3]=='p'&&in[4]=='s'&&in[5]==':'&&in[6]=='/'&&in[7]=='/'||in[0]=='h'&&in[1]=='t'&&in[2]=='t'&&in[3]=='p'&&in[4]==':'&&in[5]=='/'&&in[6]=='/')
    {
    	...
    	system("cls");
    	if (l == 'C')
        	cout <<"检测到您输入了网址,是否要直接访问该网址?\n\nF7:是    F8:否\n\nF1:是(记住本次选择)    F2:否(记住本次选择)";
    	if (l == 'S')
     	   	cout << "Detected that you have entered a URL, do you want to visit the URL directly?\n\nF7:Yes F8:No\n\nF1: Yes (remember this selection) F2: No (remember this selection)";
  	 	if (l == 'K')
      	  	cout << "Detected that you have entered a URL, do you want to visit the URL directly?\n\nF7:Yes F8:No\n\nF1: Yes (remember this selection) F2: No (remember this selection)";
  	 	if (l == 'J')
        	cout << "URLを入力したことが検出されました。そのURLに直接アクセスしますか?\n\nF7:はい    F8:いいえ\n\nF1：はい（この選択を覚えておいてください）F2：いいえ（この選択を覚えておいてください）";
        while (true)
		{
